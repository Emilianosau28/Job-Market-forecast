...
if __name__ == "__main__":
    ...
    # ensure month is timestamp (not Period)
    ...
    for col in df_feat.columns:
        if col.startswith(f"{TARGET}_lag_") or col.startswith(f"{TARGET}_rolling_average_"):
            features.append(col)
    features.append("t")
    ...
    for step in range(HORIZON_MONTHS):
        ...
        # ensure numeric target column
        df_roll[TARGET] = pd.to_numeric(df_roll[TARGET], errors="coerce")

        # rebuild features with new row included
    ...
    engine = get_engine()
    forecast_df.to_sql(
        "forecast_hourly_earnings_monthly",
        engine,
        if_exists="replace",
        index=False
    )
